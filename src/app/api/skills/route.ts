...
export async function GET(request: Request) {
  ...
  try {
    const skillsRes = await fetchNotionData(DATABASE_IDS.skills, [
      { property: "category", direction: "ascending" },
      { property: "name", direction: "ascending" },
    ])
    const data = skillsRes.results

    if (lang !== "ko") {
      const translationMap = await getGlobalTranslations({ skills: data }, lang)
      data.forEach((item: Record<string, unknown>, itemIndex: number) => {
        applyTranslations(item, "skills", itemIndex, translationMap)
      })
    }
